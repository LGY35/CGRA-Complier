...
void alg3_func(unsigned short * d1in, unsigned short  * d1out, unsigned int width, unsigned int height)
{
	...
	for (int c = 0; c < 4; c++)       
	{
		...
		for (int i = 0; i < (int)height / 2; i++)//d1_c to d1_ce full mapping
			for (int j = 0; j < (int)width / 2; j++)
				d1_ce[(4 + i) * (width / 2 + 8) + 4 + j + c * (width / 2 + 8) * (height / 2 + 8)] 
				= d1_c[i *  width / 2 + j + c * (width / 2) * (height / 2)];

		for (int i = 4; i < (int)height / 2 + 4; i++)//leftmost padding: (4 ~ 1/2hight+4)*(0~4)
			for (int j = 0; j < 4; j++)
				d1_ce[i * (width / 2 + 8) + j + c * (width / 2 + 8) * (height / 2 + 8)] 
				= d1_c[(i - 4) *  width / 2 + c * (width / 2) * (height / 2)];

		for (int i = 4; i < (int)height / 2 + 4; i++)//rightmost padding: (4 ~ 1/2hight+4)*(1/2width+4~1/2width+8))
			for (int j = width / 2 + 4; j < (int)width / 2 + 8; j++)
				d1_ce[i * (width / 2 + 8) + j + c * (width / 2 + 8) * (height / 2 + 8)] 
				= d1_c[(i - 4) *  width / 2 + width / 2 - 1 + c * (width / 2) * (height / 2)];

		for (int i = 0; i < 4; i++)//uppermost padding: 4-> *(0~4))
			for (int j = 0; j < (int)width / 2 + 8; j++)
				d1_ce[i * (width / 2 + 8) + j + c * (width / 2 + 8) * (height / 2 + 8)] 
				= d1_ce[4 * (width / 2 + 8) + j + c * (width / 2 + 8) * (height / 2 + 8)];

		for (int i = height / 2 + 4; i < (int)height / 2 + 8; i++)//bottommost padding: (1/2hight+4~1/2hight+8)*()
			for (int j = 0; j < (int)width / 2 + 8; j++)
				d1_ce[i * (width / 2 + 8) + j + c * (width / 2 + 8) * (height / 2 + 8)] 
				= d1_ce[(height / 2 + 3) * (width / 2 + 8) + j + c * (width / 2 + 8) * (height / 2 + 8)];
	}
