			...
			for (int j = 6; j < (int)width / 2 + 6; j++)
			{
				...
				for (int m = -4; m <= 4; m++)
				{
					for (int n = -4; n <= 4; n++)
					{
						...
						weight = (sad > sad_thr) ? 0 : (sad_thr - sad);//25 拍执行一次
						weight_sum += weight;
						filter_sum += weight * ref_block[2][2];
					}
				}
				weight_sum += sad_thr;
				filter_sum += sad_thr * central_block[2][2];//每80*25拍执行一次
